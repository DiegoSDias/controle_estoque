...
const VendasList = ({ clientes, produtos }) => {
  ...
  const abrirModalEdicao = async (id_venda) => {
    try {
      ...
      const itensCarrinho = (data.itens ?? [])
        // ignora itens devolvidos (quantidade 0) no carrinho de edição
        .filter((it) => it.quantidade > 0)
        .map((it) => {
          const prodInfo = produtos.find((p) => p.id_produto === it.id_produto) || {};
          return { 
            ...prodInfo, 
            id_item_venda: it.id_item_venda,          // importante pra devolução
            id_produto: it.id_produto, 
            nome_produto: it.nome_produto, 
            quantidade: it.quantidade,
            preco_venda: it.preco_unitario,
            devolvido: !!it.devolvido,               // vem do banco, se quiser usar
          };
        });
      ...
      if (delta > dispAtual) {
        ...
        return prevCarrinho.map((i) =>
          i.id_produto === id_produto
            ? { ...i, quantidade: qtdAntiga + dispAtual }
            : i
        );
      }
      return prevCarrinho.map((i) =>
  ...
  const abrirModalDevolucao = (item) => {
    setItemDevolucao(item);
    setQtdDevolver(item.quantidade || 1);
    setMotivoDevolucao("");
  ...
  const confirmarDevolucao = async () => {
    ...
    if (qtd <= 0 || qtd > itemDevolucao.quantidade) {
      alert(
        `Quantidade deve estar entre 1 e ${itemDevolucao.quantidade}.`
      );
      return;
    }

    try {
      // Se a venda já existir, registra devolução no backend
      if (vendaEmEdicao) {
        setSalvandoDevolucao(true);
        const resp = await fetch(`${API_URL}/devolucoes`, {
          method: "POST",
        ...
        if (!resp.ok) {
          ...
          throw new Error(
            err.erro || "Falha ao registrar devolução."
          );
        }
      }
      ...
      // Atualiza lista e KPIs se for venda já gravada
      if (vendaEmEdicao) {
        carregarVendas();
        carregarStats();
      }
      ...
      console.error(e);
      alert(e.message || "Erro ao registrar devolução.");
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <KpiCard
          ...
          value={fmtBRL(stats.faturado_periodo)}
          icon={DollarSign}
        ...
        <KpiCard
          title="Ticket Médio"
          value={fmtBRL(stats.ticket_medio_periodo)}
          icon={DollarSign}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-x-auto">
        ...
        {vendas.length === 0 && (
          <p className="p-6 text-center text-gray-500">
            Nenhuma venda encontrada para este período ou busca.
          </p>
        )}
      ...
      {isDevolucaoOpen && itemDevolucao && (
        <div className="modal-overlay" onClick={fecharModalDevolucao}>
          <div
            className="modal-content max-w-lg"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="text-2xl font-bold mb-4">Devolução de item</h2>
            ...
            <div className="space-y-4">
              ...
              <label className="flex items-center gap-2 text-sm">
                ...
                />
                <span>Item pode voltar para o estoque (reutilizável)</span>
              </label>
